...
def get_touch_idx(close, events, ptsl, molecule=None):
    ...
    ptsl: list
        Coefficients of width of profit taking and stop loss.
        ptsl[0] and ptsl[1] correspond to width of profit taking
        and stop loss respectively. Both values have to be positive.
    ...
    if ptsl[0] > 0:
        pts = ptsl[0] * _events["trgt"]
    else:
        # Switch off profit taking
        pts = pd.Series(index=_events.index)
    if ptsl[1] > 0:
        sls = -ptsl[1] * _events["trgt"]
    else:
    ...
    for loc, t1 in vertical_lines.iteritems():
        ...
        touch_idx.at[loc, 'pt'] = df[df > pts[loc]].index.min()
    touch_idx['t1'] = _events['t1'].copy(deep=True)
    return touch_idx


def get_events(close, timestamps, ptsl, trgt, min_ret=0,
